...
class Puzzle12Visualization(Scene):
    def construct(self):
        ...
        for name in ["a", "b"]:
            ...
            array_label = Text(
                f"Input Vector {name.upper()} (size=8)", font_size=18
            )
        ...
        output_label = Text("Output (size=1)", font_size=18).next_to(
        ...
        while stride > 0:
            ...
            for i in range(stride):
                ...
                h1 = Square(
                    side_length=cell_size, stroke_color=YELLOW, fill_opacity=0.2
            ...
            active_text = Text(
                f"Active elements: {stride * 2}", font_size=14, color=YELLOW
            )
        ...
        self.play(FadeIn(final_arrow), FadeIn(output_highlight))
        self.wait(60)
