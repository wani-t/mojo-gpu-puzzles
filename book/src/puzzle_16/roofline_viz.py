...
class RooflineModelVisualization(Scene):
    def construct(self):
        ...
        # Title - larger font
        title = Text(
            "The Roofline Model", font_size=36, color=YELLOW, weight=BOLD
        )
        ...
        for pos, val in zip(x_major_positions, x_major_values, strict=False):
            main_label = Text(f"{val}", font_size=20, color=WHITE)
            main_label.set_stroke(BLACK, width=2, background=True)
            log_notation = Text(
                f"10^{int(np.log10(val))}", font_size=14, color=GRAY_B
            )
        ...
        for pos, val in zip(y_major_positions, y_major_values, strict=False):
            if val >= 1000:
                if val >= 100000:
        ...
        self.play(Write(x_tick_labels), Write(y_tick_labels))
        self.wait(1)
        ...
        intro_text = Text(
            "Performance is limited by:", font_size=22, color=WHITE, weight=BOLD
        )
        intro_text.to_edge(RIGHT, buff=0.5).shift(UP * 2.5)

        compute_bullet = Text(
            "• Compute (Arithmetic Units)", font_size=18, color=BLUE_D
        )
        ...
        memory_bullet.next_to(
            compute_bullet, DOWN, aligned_edge=LEFT, buff=0.15
        )
        ...
        compute_roof = axes.get_horizontal_line(
            axes.c2p(2.5, compute_roof_y), color=BLUE_D, stroke_width=4
        )

        compute_label = Text(
            "P_peak = 19.5K GFLOP/s", font_size=24, color=BLUE_D
        )
        ...
        memory_roof_points = [
            axes.c2p(x, y)
            for x, y in zip(x_range_roof, y_range_roof, strict=False)
        ]
        ...
        memory_equation.next_to(
            axes.c2p(0.3, 2.5), RIGHT + DOWN * 0.5, buff=0.1
        )
        ...
        self.play(
            Create(memory_roof), Write(memory_equation), Write(bandwidth_label)
        )
        self.wait(1)
        ...
        critical_bg = BackgroundRectangle(
            critical_label, color=BLACK, fill_opacity=0.7, buff=0.1
        )
        _critical_group = VGroup(critical_bg, critical_label)
        ...
        puzzle_title = Text(
            "Our Matrix Multiplication:", font_size=20, color=WHITE, weight=BOLD
        )
        ...
        naive_details = Text(
            "Naïve: I = 0.1875 FLOP/B", font_size=18, color=ORANGE
        )
        naive_details.next_to(puzzle_title, DOWN, aligned_edge=LEFT, buff=0.3)

        naive_perf = Text(
            "Performance: ~ 292 GFLOP/s", font_size=16, color=ORANGE
        )
        ...
        puzzle_shared_x = 0.4
        puzzle_shared_point = axes.c2p(
            puzzle_shared_x, np.log10(puzzle_shared_P)
        )
        puzzle_shared_dot = Dot(puzzle_shared_point, color=TEAL, radius=0.1)

        shared_details = Text(
            "Shared Memory: I = 0.375 FLOP/B", font_size=18, color=TEAL
        )
        shared_details.next_to(naive_perf, DOWN, aligned_edge=LEFT, buff=0.2)

        shared_perf = Text(
            "Performance: ~ 583 GFLOP/s", font_size=16, color=TEAL
        )
        ...
        self.play(
            Write(shared_details), Write(shared_perf), Create(puzzle_shared_dot)
        )
        ...
        improvement_text = Text(
            "2x faster!", font_size=16, color=YELLOW, weight=BOLD
        )
        improvement_text.next_to(improvement_arrow.get_center(), UP, buff=0.2)

        self.play(Create(improvement_arrow), Write(improvement_text))
        self.wait(3)

        # Step 9: Show optimization strategies
        strategies_title = Text(
            "Optimization Strategies:", font_size=18, color=WHITE, weight=BOLD
        )
        strategies_title.next_to(shared_perf, DOWN, aligned_edge=LEFT, buff=0.3)

        strategy1 = Text(
            "• Increase Arithmetic Intensity", font_size=16, color=GREEN_D
        )
        ...
        strategy3 = Text(
            "• Data Reuse. Stay Tuned!", font_size=16, color=GREEN_D
        )
        ...
        goal_text = Text(
            "Goal: Reach the roof!", font_size=18, color=GOLD, weight=BOLD
        )
        goal_text.next_to(goal_point, UP + LEFT, buff=0.2)

        self.play(Create(goal_arrow), Write(goal_text))
